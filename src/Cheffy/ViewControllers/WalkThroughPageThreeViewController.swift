...
class WalkThroughPageThreeViewController: UIViewController {
    ...
    @IBAction func btnGetStarted_action(_ sender: Any) {
        // Navigate to the login page (LoginPageViewController)
        if let vc = storyboard?.instantiateViewController(withIdentifier: "LoginPageViewController") as? LoginPageViewController {
            navigationController?.pushViewController(vc, animated: false)
        } else {
            print("Failed to instantiate LoginPageViewController")
        }
    }
}
